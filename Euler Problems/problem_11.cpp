...
//Prefixing with 0 appears to be octal. Shouldnt matter
	...
	for (int i = 0; i < 400; i++) {
		...
		if (x < 17) {
			for (int k = 0; k < 4; k++) {
				...
				if (y < 17) 
					currentValue[0] *= numbers[i + k * 20 + k];
		
				currentValue[1] *= numbers[i + k];
			}
			if (currentValue[0] > currentValue[1])
				currentValue[1] = currentValue[0];

			if (currentValue[1] > maximum) {
				maximum = currentValue[1];
				index = i;
				value = numbers[i];
			}
